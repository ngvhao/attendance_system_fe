...
interface ModalContextType {
  ...
  setOpen: (modal: React.ReactNode, fetchdata?: () => Promise<any>, modalId?: string) => void;
...
const ModalProvider: React.FC<ModalProviderProps> = ({ children }) => {
  ...
  const setOpen = async (modal: React.ReactNode, fetchdata?: () => Promise<any>, modalId: string = 'default') => {
    if (fetchdata) {
    ...
    setCanCloseState((prev) => (prev[modalId] === true ? prev : { ...prev, [modalId]: true }));
  };
  ...
  return (
    <ModalContext.Provider value={{ data, isOpen, canClose, setOpen, setClose, setCanClose }}>
      {children}
      {Object.entries(modals).map(([id, modal]) => isOpen[id] && <React.Fragment key={id}>{modal}</React.Fragment>)}
